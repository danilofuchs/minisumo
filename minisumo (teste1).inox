...
int distance = 60; // essa variável será o valor padrão máximo para distância ao adversário. Serve para não detectar coisas fora do tatame.
int IRFrenteE; // variáveis que servirão para identificação da cor e evitar a queda do robo do tatame.
int IRTras;               
int IRFrenteD;
int tempo = 0;
int testeE = 0, testeD = 0;
int botao = 0;
int tempoFuga =400;
long aleatorio=1;
int cont=0, contcf1 = 0, contcf2 = 0, contt = 0;
#define pinBotao 7
...
void setup() {
  ...
  enum ESTADO {Fugindo, Procurando, Atacando, Teste};
...
void loop() {
  ...
  else if (distanceD < distance && distanceE < distance) { // adversário à frente
    andarFrente(255); // aqui ele ataca.
    //Serial.println("atacando");
    estado = Atacando;
    delay(tempo);
  }
  else if (distanceD < distance && millis()-ultimoTempo>tempoFuga) { // se isso for verdade, quer dizer que distanceE>distance.
    girarDireita(255);
    //Serial.println("alvo a direita");
    estado = Procurando;

    if( millis() - testeE < 500){
      if(contt >= 4)
      {
       andarFrente(255);
       delay(300); 
  ...
  else if (distanceE < distance && millis()-ultimoTempo>tempoFuga) { // se isso for verdade, quer dizer que distanceD>distance.
    girarEsquerda(255);
    estado = Procurando;
    //Serial.println("alvo a esquerda");

    if( millis() - testeD < 500){
      if(contt >=4)
      {
       andarFrente(255);
       delay(300); 

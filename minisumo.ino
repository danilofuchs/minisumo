...
{ // não definir limites de parada, vamos fazer no loop (estratégia básica)
...
float lerUltraE()
{ //leitura da distancia com média ponderada entre 5 leituras
...
void loop()
{
  ...
  else if (distanceD < distance && distanceE < distance && !precisaAtacar)
  {                   // adversário à frente
  ...
  else if (millis() - ultimoTempo > tempoFuga)
  { // se ele chegar nessa condição, quer dizer que as duas distancias são maiores que distance, então ele deve fazer algo para procurar o adversário.
